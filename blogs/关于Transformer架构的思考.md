...
通常迭代设计重点集中的模块和**代表性的改动和改动目标**在下面的范围里

| 改动的位置/模块                              | 改动目标         | 改动方案/思路                                                                              | 优缺点/针对的痛点                                              |
|:--------------------------------------|:-------------|:-------------------------------------------------------------------------------------|:-------------------------------------------------------|
| **整体架构的形态**                           | 场景适配         | **Encoder-Only适合理解类任务** <br> **Decoder-Only适合生成类任务** <br> **Encoder-Decoder序列到序列任务** | 根据具体任务场景（如分类、翻译、对话）选择最高效适合的范式。Decoder-Only已成为主流选择      |
| **Attention(自注意力机制)** <br>主要负责捕捉上下文关联 | 信息交互效率、长文本能力 | **共享KV头减少GPU占用并加速推理**<br> **局部注意力如滑动窗口**<br> **线性注意力降低复杂度**<br>**系统层感知硬件优化IO**       | 降低KVCache的显存占用，并将计算复杂度从指数级别降低到近线性                      |
| **Position(位置编码)** <br>主要负责标识序列顺序     | 顺序表达、长度扩散能力  | **绝对位置编码**<br>**相对位置编码**<br>**旋转位置编码**                                               | 解决模型性能问题，旋转位置编码目前兼顾了绝对和相对位置优势。                         |
| **FFN(前馈神经网络)** <br>主要负责非线性特征变换       | 强化表达能力、非线性激活 | **增加非线性表达**<br>**调整FFN的隐藏层维度**                                                       | 在计算量增加不大的情况下能提供更强的梯度间的流动能力和拟合能力，提升模型指标                 |
| **Norm(归一化层)** <br> 主要负责稳定数值分布        | 训练稳定性        | **变体替换**                                                                             | 梯度消失和爆炸问题，解决不稳定性                                       |
| **稀疏化**                               | 高效扩大有效参数量    | **混合专家模型**                                                                           | 打破了算力与参数量强绑定的限制，在不增加推理/训练的情况下能成倍/十倍规模的扩大模型的总参数量来捕获更多知识 |

当前的开源大模型（如Qwen、DeepSeek、LLaMA等）的底层架构，都有一套自适的**最佳实践组合**落地，也形成了一定程度的标准。
