...
对于db的每一次变更都要做好收口以及可解释和追溯的能力。当然实际上不止支付业务，任何业务都有一样的特性，只是离钱更近的业务对于**确定性**体感更强烈。
...
到AI模型，当深度学习模型的规模（参数量、数据量、计算量）跨越某个阈值后，系统展现出其小规模版本所不具备的、没有被明确目标训练过的复杂且不是被编程进去的高级能力。**是系统自己长出来的能力**。
...
实际中要结合的手段才是核心重点。好像写一个hello world和应对企业级工程的开发都是会编程，但是中间可能差着一个维度。
