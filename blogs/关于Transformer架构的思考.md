...
| 改动的位置模块       | 改动目标         | 具体思路与改动方案                                                                                                   | 优缺点 / 解决的痛点                                               |
|:--------------|:-------------|:------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------|
| **架构形态**      | 任务范式适配       | **Encoder-Only**：适合理解类任务 <br> **Decoder-Only**：适合生成类任务 <br> **Encoder-Decoder**：序列到序列任务                     | 根据具体任务（如分类、翻译、对话）选择最高效的范式。Decoder-Only已成为大模型的绝对主流。        |
| **Attention** | 信息交互效率、长文本能力 | **共享KV头减少显存资源占用并加速推理**<br> **稀疏/局部注意力如滑动窗口注意力**<br> **线性注意力通过核函数等方法降低复杂度**<br>**系统层FlashAttention感知硬件优化IO** | 降低KVCache的显存占用，并将计算复杂度从指数级别降低到近线性。                        |
| **Position**  | 顺序表达、长度扩散能力  | **绝对位置编码**<br>**相对位置编码**<br>**旋转位置编码**                                                                      | 解决模型性能问题。旋转位置编码目前为兼顾了绝对和相对位置优势的标配。                        |
| **FFN**       | 强化表达能力、非线性激活 | **增加非线性表达**<br>**调整FFN的隐藏层维度**                                                                              | 在计算量增加不大的情况下能提供更强的梯度间的流动能力和拟合能力，提升模型指标。                   |
| **Norm**      | 稳定训练         | **变体替换**                                                                                                    | 梯度消失和爆炸问题，解决不稳定性                                          |
| **稀疏化**       | 高效扩大有效参数量    | **混合专家模型(MoE)**                                                                                             | 打破了算力与参数量强绑定的限制，能够在不增加推理/训练的情况下，成倍/十倍规模的扩大模型的总参数量来捕获更多知识。 |

当前的开源大模型（如Qwen、DeepSeek、LLaMA等）的底层架构，都有一套自适的**“最佳实践组合”**落地，也形成一定程度的标准。
...
发表过相类似的声音，并且情绪色彩更浓厚称目前产业都围绕在Transformer架构的错误方向进行大量的资源浪费，认为AI领域已僵化，正错失下一个突破。

我认为他们这种学者精神也好研究精神也好，或者说是追求真理的精神，是值得人敬佩和学习的品质。不仅没有守着固有成果坐享不放，冷静且客观充满对知识和真理的探寻精神。

历史一定是持续迭代的，世界也不是非黑即白，总有先行者不断突破世界的边界。Transformer或许是目前为止适配性最好的终极答案，更可能是未来无限可能中的里程碑之一。