...
|:----------------------------------|:-------------|:-------------------------------------------------------------------------------------|:-------------------------------------------------------|
| **架构形态**                          | 任务范式场景适配     | **Encoder-Only适合理解类任务** <br> **Decoder-Only适合生成类任务** <br> **Encoder-Decoder序列到序列任务** | 根据具体任务（如分类、翻译、对话）选择最高效适合的范式。Decoder-Only已成为大模型的主流选择    |
| **Attention(自注意力机制)** 主要负责捕捉上下文关联 | 信息交互效率、长文本能力 | **共享KV头减少GPU占用并加速推理**<br> **局部注意力如滑动窗口**<br> **线性注意力降低复杂度**<br>**系统层感知硬件优化IO**       | 降低KVCache的显存占用，并将计算复杂度从指数级别降低到近线性                      |
| **Position(位置编码)** 主要负责标识序列顺序     | 顺序表达、长度扩散能力  | **绝对位置编码**<br>**相对位置编码**<br>**旋转位置编码**                                               | 解决模型性能问题，旋转位置编码目前兼顾了绝对和相对位置优势。                         |
| **FFN(前馈神经网络)** 主要负责非线性特征变换       | 强化表达能力、非线性激活 | **增加非线性表达**<br>**调整FFN的隐藏层维度**                                                       | 在计算量增加不大的情况下能提供更强的梯度间的流动能力和拟合能力，提升模型指标                 |
| **Norm(归一化层)**  主要负责稳定数值分布        | 训练稳定性        | **变体替换**                                                                             | 梯度消失和爆炸问题，解决不稳定性                                       |
