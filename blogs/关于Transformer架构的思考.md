...
|:--------------|:-------------|:------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------|
| **架构形态**      | 任务范式适配       | **Encoder-Only**：适合理解类任务 <br> **Decoder-Only**：适合生成类任务 <br> **Encoder-Decoder**：序列到序列任务                     | **优点**：根据具体任务（如分类、翻译、对话）选择最高效的范式。Decoder-Only已成为大模型的绝对主流。        |
| **Attention** | 信息交互效率、长文本能力 | **共享KV头减少显存资源占用并加速推理**<br> **稀疏/局部注意力如滑动窗口注意力**<br> **线性注意力通过核函数等方法降低复杂度**<br>**系统层FlashAttention感知硬件优化IO** | **优点**：降低KVCache的显存占用，并将计算复杂度从指数级别降低到近线性。                        |
| **Position**  | 顺序表达、长度扩散能力  | **绝对位置编码**<br>**相对位置编码**<br>**旋转位置编码**                                                                      | **痛点解决**：解决模型性能问题。旋转位置编码目前为兼顾了绝对和相对位置优势的标配。                      |
| **FFN**       | 强化表达能力、非线性激活 | **门控线性单元增加非线性表达**<br>**调整FFN的隐藏层维度**                                                                        | **优点**：在计算量增加不大的情况下能提供更强的梯度间的流动能力和拟合能力，提升模型指标。                   |
| **Norm**      | 稳定训练         | **变体替换**                                                                                                    | **痛点**：梯度消失和爆炸问题，解决不稳定性                                          |
| **稀疏化**       | 高效扩大参数量      | **混合专家模型(MoE)**                                                                                             | **优点**：打破了算力与参数量强绑定的限制，能够在不增加推理/训练的情况下，成倍/十倍规模的扩大模型的总参数量来捕获更多知识。 |
