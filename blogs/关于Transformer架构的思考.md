...
这个公式是在软件工程专业学习的第一课内容可以说是认知基础，是一种基准范式。而且后面的学习内容也基本都是在这上展开的。
...
同时对于数据的任何变更都要做清晰，且具备**可解释和追溯**的能力。用支付业务举例体感更直接，实际任何业务都有一样对**确定性**
的要求。
...
这种方式存在的问题比如无法捕捉远距离长期依赖，需要按顺序处理无法进行并行计算。虽然过程有[LSTM](https://en.wikipedia.org/wiki/Long_short-term_memory)
等方法做针对优化，但是本质还是在顺序的基础上，解决的程度有限没能根本的解决问题。
...
Transformer[论文](https://arxiv.org/abs/1706.03762?utm_source=chatgpt.com)
诞生在2017年，是一种特殊深度学习模型结构，核心是引入自注意力机制。Transformer架构中，给了每个词向量一个**位置编码**
，再通过预先训练出的权重值经过一系列计算，
得到其他词的上下文信息。
...
理解了Transformer大体的工作机制，那么看下来Transformer的优势包括

1. 并行计算能力，更充分利用硬件资源
2. 捕捉长距离依赖，自注意力机制让每个位置的输入可以与其他信息位置进行交互和向量存留
3. 灵活可扩展，模块职责清晰，无论是数据还是功能上都具备高扩展性

---

### F、Transformer架构的经典变式

Transformer架构的各个组件功能职责清晰，所以在实际很多落地的架构对Transformer架构做了一些变式设计，来做针对性优化设计。
有针对哪些方面呢？所有的优化基本都是有的放矢，都在Transformer整体职责划分的架构基础范围内。

通常迭代设计重点集中的模块和**代表性的改动和改动目标**在下面的表格范围里

| 改动的位置/模块        | 模块功能职责        | 改动目标         | 改动方案/思路                                                                                                                                                                                                                                             | 优缺点/针对的痛点                                          |
|:----------------|:--------------|:-------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------|
| 整体架构的形态         | /             | 场景适配         | Encoder-Only适合理解类任务；<br> Decoder-Only适合生成类任务；<br> Encoder-Decoder序列到序列任务                                                                                                                                                                            | 根据具体任务场景（如分类、翻译、对话）选择最高效适合的范式。Decoder-Only目前是主流选择  |
| Attention自注意力机制 | 模块主要负责捕捉上下文关联 | 信息交互效率、长文本能力 | 共享KVCache，合理利用模型token的[局部性原理](https://github.com/BBLLMYD/blog/blob/master/blogs/%E6%8A%BD%E8%B1%A1%E4%B9%8B%E4%BA%8E%E2%80%9C%E5%B1%80%E9%83%A8%E6%80%A7%E5%8E%9F%E7%90%86%E2%80%9D.md)，减少GPU占用加速推理；<br> 局部注意力如滑动窗口；<br> 线性注意力降低复杂度；<br>系统层优化感知硬件IO | 降低KV对显存占用，降低计算复杂度                                  |
| Position位置编码    | 主要负责标识顺序      | 顺序表达、长度扩散能力  | 绝对位置编码<br>相对位置编码<br>旋转位置编码                                                                                                                                                                                                                          | 解决模型性能问题，旋转位置编码目前兼顾了绝对和相对位置优势。                     |
| FFN前馈神经网络       | 负责非线性特征变换     | 非线性激活强化表达能力  | 增加非线性表达、调整FFN的隐藏层维度                                                                                                                                                                                                                                 | 在计算量增加不大的情况下能提供更强的梯度间的流动能力和拟合能力，提升模型指标             |
| Norm归一化         | 稳定数值分布        | 训练稳定性        | 替换变体                                                                                                                                                                                                                                                | 解决不稳定性如梯度消失和爆炸                                     |
| 稀疏化             | /             | 高效扩大有效参数量    | 混合专家模型                                                                                                                                                                                                                                              | 改变了算力与参数量强绑定的限制，在不增加推理/训练的情况下能倍规模的扩大模型的总参数量来获取更多知识 |

当前的开源大模型（如Qwen、DeepSeek等）的底层架构，都有一套自适的**最佳实践组合**落地，也形成了一定程度的标准。


---

### G、Transformer作为行业的事实标准

Transformer已经成为行业内不争的事实标准， Transformer可以理解为**一种可规模化的信息交互机制的可落地方案**
，并且Transformer的架构清晰、灵活可扩展，够适应不同的场景需求，
这种灵活性和生态让架构本身在未来的发展演变过程仍然持续具有很宽阔的成长空间。

> Transformer is not the final answer / not the future architecture of AI

这是Transformer的作者之一[Ashish Vaswani](https://www.linkedin.com/in/ashish-vaswani-99892181)说的，
同为Transformer作者之一的[Llion Jones](https://www.linkedin.cn/incareer/in/llion-jones-9ab3064b)
发表过相类似的声音，并且情绪色彩更浓厚称“目前产业都围绕在Transformer架构的错误方向进行大量的资源浪费，AI领域已僵化，正错失下一个突破”。

首先他们的身份在行业领域内已然足够权威，且不说观点是否客观或者是否偏激，我认为他们这种学者精神也好研究精神也好，或者说是追求真理的精神，是值得人敬佩和学习的品质。不仅没有守着固有成果坐享，冷静且客观充满对知识和真理的探寻精神。

历史一定是持续迭代的，观点也不是非黑即白，总有先行者不断突破世界的边界。

Transformer或许是目前为止适配性最好的终极答案，更可能是未来无限可能途中的里程碑之一。